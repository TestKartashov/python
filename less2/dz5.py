...
# Пользователь ввел число 8. Результат: 8, 7, 5, 3, 3, 2. !11
...
i = len(lis)
while True:
    inp = input("Enter number, for Exit enter 'q': ")
    if inp == "q":
        break
    inp_int = int(inp)
    ...
    for index, l in enumerate(lis):
        ...
        if inp_int > l:
            break
    # для проверки что вставляется после повторяющихся можно использовать inp
    lis.insert(i, inp_int)
    print(lis, i)
